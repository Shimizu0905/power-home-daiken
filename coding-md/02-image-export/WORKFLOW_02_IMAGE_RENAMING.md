...
# 例: interview ページの場合
./scripts/resize-images-2x.sh src/images/interview/
```
...
# 例: interview ページで使用されている画像のハッシュ名
...
# 例: interview ページの場合（メンバーの顔写真のみ使用、SVGは使用されていない）
cd src/images/interview/
# 使用されていないSVGファイルを削除
rm -f *.svg
...
**メンバーの顔写真の場合（jpg 形式に変換）:**

```bash
# 1. PNGファイルをコピーして一時ファイルを作成
cp {画像ディレクトリ}/{ページ名}/{ハッシュ名}.png {画像ディレクトリ}/{ページ名}/member_01.png

# 2. PNGからJPGに変換（macOSの場合）
sips -s format jpeg {画像ディレクトリ}/{ページ名}/member_01.png --out {画像ディレクトリ}/{ページ名}/member_01.jpg

# 3. 一時のPNGファイルを削除
rm {画像ディレクトリ}/{ページ名}/member_01.png

# 例: interview ページの場合
cp src/images/interview/{ハッシュ名}.png src/images/interview/member_01.png
sips -s format jpeg src/images/interview/member_01.png --out src/images/interview/member_01.jpg
rm src/images/interview/member_01.png
```
...
# リネーム済みのファイルを確認
cd {画像ディレクトリ}/{ページ名}/
ls -1 member_*.jpg {セクション名}_*.png common_*.{png,svg} 2>/dev/null
